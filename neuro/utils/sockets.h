...
inline int SocketServe(int port)
{
    int sock;
    ...
    if(::bind(sock, (struct sockaddr *) &serv_addr, sizeof(serv_addr))) {
        perror("SocketServe: bind()");
        exit(1);
    }
    return sock;
}

inline int AcceptConnection(int sock)
{
    if(listen(sock, 500)) {
        perror("AcceptConnection: listen()");
        exit(1);
    }
...
inline int SocketAccept(int port)
{
    int sock;
    int fd;

    sock = SocketServe(port);
    fd = AcceptConnection(sock);
    close(sock);
    return fd;
}
...
/*
    ...
    Example usage:
        ...
        if (fgets(buf, 1000, fin) != NULL)
            ...
            printf("Connection closed\n");
*/
inline int SocketRequest(const char* host, int port)
{
    int fd;
    struct hostent *he;
    struct sockaddr_in serv_addr;
